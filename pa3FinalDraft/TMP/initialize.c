...
/* --------------------- initilization for pa3 ----------- */
         
        init_bsm();  // initialize primar tables
        init_secbsm(); // initialize secondary table
        init_frm();
        rc = init_pt(); // initialization of page tables
        init_ptr(); //initialize pointer for SC
        if (rc == SYSERR) {
           kprintf("\n could not initialize page tables ");
           return(SYSERR);
        }
        pd_t *pd = pd_create(0); // create a page directory for the null process
        unsigned long adr = (unsigned int) pd;
        
        set_PDBR(adr);
        set_evec(14, (unsigned long) pagefaulthndlr);
        
        
        enable_paging(); //set cr0 register to enable paging 
...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	pptr->pprio = 0;
	currpid = NULLPROC;
 	pptr->pdbr = adr;       // setting the pdbr value for null process to adr
	...
	rdytail = 1 + (rdyhead=newqueue());/* initialize ready list */
        ...
        // initialize the primary and secondary backgin store
